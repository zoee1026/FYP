...
def build_point_pillar_graph(params: Parameters, batch_size: int = Parameters.batch_size):
    ...
    input_pillars = tf.keras.layers.Input(input_shape, batch_size=2, name="pillars/input")
    input_indices = tf.keras.layers.Input((max_pillars, 3), batch_size=2, name="pillars/indices",
                                          dtype=tf.int32)
