...
class KittiDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, train_data_path=None,test_data_path=None, logger=None):
        """
        Args:
            root_path:
            dataset_cfg:
    ...
    def get_lidar(self, lidar_file_path):
        # lidar_file = self.root_split_path / 'velodyne' / ('%s.bin' % idx)
        # assert lidar_file.exists()
        return np.fromfile(str(lidar_file_path), dtype=np.float32).reshape(-1, 4)
...
def create_kitti_infos(dataset_cfg, class_names, train_data_path, test_data_path, save_path, workers=4):
    dataset = KittiDataset(dataset_cfg=dataset_cfg,
                           class_names=class_names, train_data_path=train_data_path, test_data_path=test_data_path, training=False)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        ROOT_DIR = '/media/sdb1/zoe/FYP/OpenPCDet/'
        create_kitti_infos(
            dataset_cfg=dataset_cfg,
            class_names=['Car', 'Pedestrian', 'Cyclist'],
            train_data_path='/media/sdb1/zoe/FYP/train_files/',
            test_data_path='/media/sdb1/zoe/FYP/eval_files/',
            save_path='/media/sdb1/zoe/FYP/OpenPCDet/data/custom/'
        )
