...
def preprocess_true_boxes(labels: List[Label3D],knn ):
        ...
        #   中心点(m,n,2) 宽高(m,n,2)
        #-----------------------------------------------------------#
        boxes_xy = objectPositions[...,:2]
        boxes_wh =  objectDimensions[...,:2]
        #-----------------------------------------------------------#
        #   将真实框归一化到小数形式
        #-----------------------------------------------------------#
        ...
        for b in range(nbObjects):
            ...
            #-------------------------------------------------------#
            ratios_of_gt_anchors = np.expand_dims(wh, 1) / np.expand_dims(anchors[...,:2], 1)
            ratios_of_anchors_gt = np.expand_dims(anchors[...,:2], 1) / np.expand_dims(wh, 1)
            ...
            for t, ratio in enumerate(max_ratios):
                ...
                for l in range(num_layers):
                    for k, n in enumerate(anchors_mask[l]):
                        ...
                        offsets = get_near_points(true_boxes[b,t,0] * grid_shapes[l][1], true_boxes[b,t,1] * grid_shapes[l][0], i, j)
